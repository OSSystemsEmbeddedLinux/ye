...
and `YE_EDITOR` for pager and editor, respectively.  `YE_SOURCE_DIRS`
can be set to a colon-separated list of source or layer directories
when the automatic layer detection is not appropriate.
...
A Python 3 installation and a Yocto Project build tree.  `ye` supports
